    ...
    given("학생 정보 단건 조회 요청하는 경우") {
        `when`("'신길초'를 다니는 '김모간' 이름과 일치한 정보가 없다면") {
            then("404 Resource Not Found 예외 발생한다") {
                shouldThrow<NotFoundException> { studentRepository.findStudentByNameAndSchoolName("김모간", "신길초") }
            }
        }
        `when`("'신길초'를 다니는 '김모군' 이름의 동일한 정보 다건 조회된다면") {
            then("500 유효하지 않는 정보 예외 발생한다") {
                shouldThrow<IllegalArgumentException> { studentRepository.findStudentByNameAndSchoolName("김모군", "신길초") }
            }
        }
        `when`("'신길초'를 다니는 '김모건' 이름의 동일한 정보 단건 조회된다면") {
            val student = studentRepository.findStudentByNameAndSchoolName("김모건", "신길초").getOrNull()
